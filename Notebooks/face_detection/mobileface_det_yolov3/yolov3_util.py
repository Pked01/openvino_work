...
def get_objects(output,output_layers_params, new_frame_height_width, source_height_width, prob_threshold, is_proportional):
    objects = list()
    for idx,output_layers_param in enumerate(output_layers_params):
        out_blob = output[idx]
        layer_params = YoloParams(output_layers_param, out_blob.shape[2])
    ...
    for i in range(len(objects)):
        ...
        for j in range(i + 1, len(objects)):
            if intersection_over_union(objects[i], objects[j]) > iou_threshold:
                if objects[j]['confidence'] > objects[i]['confidence']:
